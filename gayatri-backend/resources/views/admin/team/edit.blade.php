...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in" x-data="teamMemberForm()">
    ...
    <div class="flex items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 flex items-center gap-2">
                <svg class="w-7 h-7 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                Edit Profile: {{ $member->name }}
            </h1>
            <p class="text-slate-500 text-sm mt-1">Update designation, biography, practice areas, or profile photo.</p>
        </div>
    </div>
    ...
    <!-- Form Panel -->
    <form action="{{ route('admin.team.update', $member->id) }}" method="POST" class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
        @csrf
        <!-- Hidden base64 cropped image (holds new image base64 if user crops a new one, else empty string means keep existing) -->
        <input type="hidden" name="image" x-model="croppedImageBase64">

        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <!-- Left: Avatar Image Selection and Preview -->
            <div class="flex flex-col items-center space-y-4">
                <label class="block text-sm font-bold text-slate-700 self-start md:self-center">Profile Photo</label>
                ...
                <div class="relative w-44 h-44 rounded-2xl overflow-hidden bg-slate-50 border-2 border-dashed border-slate-300 hover:border-blue-500 transition-all flex items-center justify-center cursor-pointer group shadow-sm"
                    ...
                    <!-- Fallback SVG Avatar -->
                    <div x-show="!displayImage" class="text-center p-4">
                        ...
                        <span class="text-[11px] text-slate-500 font-bold block mt-2 uppercase tracking-wide">Upload Photo</span>
                        <span class="text-[9px] text-slate-400 block mt-0.5">(Square Recommended)</span>
                    </div>

                    <!-- Cropped Image Preview -->
                    <img x-show="displayImage" :src="displayImage" class="w-full h-full object-cover" style="display: none;">

                    <!-- Hover overlay for existing preview -->
                    <div x-show="displayImage" class="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition duration-200 flex items-center justify-center" style="display: none;">
            ...
            <div class="md:col-span-2 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="role" name="role" required value="{{ old('role', $member->role) }}" placeholder="e.g. Managing Partner" class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="qualification" name="qualification" required value="{{ old('qualification', $member->qualification) }}" placeholder="e.g. B.Com, FCA, DISA" class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm">
                </div>

                <!-- Custom Tags -->
                <div>
                    <label for="tags" class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Expertise Areas / Tags</label>
                    <input type="text" id="tags" name="tags" value="{{ old('tags', $tagsString) }}" placeholder="e.g. Taxation, Audit, Corporate Law (comma separated)" class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm">
                ...
                <div>
                    ...
                    <textarea id="bio" name="bio" rows="4" placeholder="Briefly introduce their background, years of practice, or domain achievements..." class="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm">{{ old('bio', $member->bio) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="px-6 py-4 bg-slate-50 text-slate-800 flex items-center justify-between border-b border-slate-200">
                <div>
                    <h3 class="text-base font-bold text-slate-900 flex items-center gap-2">
                        <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h.01M16 16h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
...
<script>
    function teamMemberForm() {
        return {
            croppedImageBase64: '',
            displayImage: '{{ $member->image_path ? asset($member->image_path) : "" }}',
            cropModalOpen: false,
